...
const stripePayment = async (req, res) => {
  ...
  try {
    ...
    // Validation
    if (!orderData?.address || Object.keys(orderData.address).length === 0) {
      return res.json({ success: false, message: "Address is not selected" });
    }
    ...
    // Get seller data
    const sellerId = orderData.items.find((item) => item.sellerId)?.sellerId;
    const sellerData = await restaurantModel.findOne({ sellerId });
    ...
    const charges = orderData.amount - itemsTotal;
    ...
    const order = {
      ...
      status: "pending",
      sellerId,
      restoName: sellerData?.name || "",
      restoAddress: sellerData?.address || "",
    };
    ...
    if (charges > 0) {
      line_items.push({
        price_data: {
          currency: "inr",
          product_data: { name: "Delivery and other charges" },
    ...
    res.json({ success: false, message: "Order not processed" });
  }
};
